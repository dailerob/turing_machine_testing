...
- **Benchmark suites** — each in its own folder with a writeup:
  ...
  - `skolr_bench/forecast/` — SKOLR/Informer time-series benchmarks
    (ETTh1/2, ETTm1/2, ECL, Traffic, Weather, ILI). Writeup TBD; see
    `gdc_etth1_full_sweep.py` for the current best result.
...
# SKOLR forecasting on ETTh1 (~12 min on RTX 5090, fp64)
...
│   ├── owa_total.py / owa_select.py       # series-weighted OWA aggregations
...
└── skolr_bench/                           # SKOLR (ICML 2025) benchmarks
    ...
    └── forecast/                          # forecasting benchmarks (Informer-style)
        ├── informer_loaders.py            # Informer-protocol data loaders
        ...
        ├── gdc_etth1_full_sweep.py        # full leakage-free sweep example
        └── (many other experimental versions; see CLAUDE.md for context)
```
...
- **Informer paper**: Zhou et al., *Informer: Beyond Efficient
  ...
  Prophet / LSTMa / DeepAR baseline numbers. Their reference
  implementation lives at
  [zhouhaoyi/Informer2020](https://github.com/zhouhaoyi/Informer2020)
  (clone into `skolr_bench/Informer2020/` if you want to retrain
  Informer; see [`skolr_bench/forecast/README.md`](skolr_bench/forecast/README.md)).
