"""PPM-D on the HMM forecasting benchmark.

Mirrors `hpylm_eval.py` exactly so PPM numbers slot into the same
leakage-free Table-7 comparison.

Variant grid (val-tuned per cell):
  - max_depth ∈ {2, 3, 4, 6}
  - discount  ∈ {0.25, 0.5, 0.75}      (canonical PPM-D fixes 0.5;
                                         we val-tune for parity with HPYLM)
  - alpha_prior ∈ {0.01}                (Laplace smoothing on output
                                         distribution; tied at 0.01)
"""
...
OUT_CSV = os.path.join(HERE, 'ppm_results.csv')


def variants():
    return [(D, d) for D in DEPTHS for d in DISCOUNTS]


class PPMForecaster:
    def __init__(self, pool: PPMPool, alpha_prior: float = ALPHA_PRIOR):
        self.pool = pool
...
def main():
    ...
    print(f"PPM-HMM sweep: {len(tasks)} cells × {n_variants} variants × "
    ...
    fields = ['regime', 'nS', 'nA', 'seed', 'N_train', 'model',
              'max_depth', 'discount', 'alpha_prior', 'horizon',
