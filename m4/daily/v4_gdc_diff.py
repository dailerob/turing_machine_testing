"""V4: GDC-TS (absorb mode) on 1-step differences for M4 daily.

Same recipe as hourly v6 (alpha=1, theta=0, terminal_behavior='absorb',
forecast_gdc_style with renormalised expected-value extraction), but
applied to the differenced series. Forecast h diffs, cumsum onto
train[-1].
"""
...
def gdc_diff_forecast(train, window_len=14, sigma_frac=0.50, h=HORIZON):
    n = len(train)
    if n < window_len + h + 2:
...
def run_series(args):
    ...
    for cfg in CONFIGS:
        try:
            pred = gdc_diff_forecast(train, **cfg, h=h)
            sm = smape(test, pred)
        except Exception:
            sm = float('nan')
...
def main():
    ...
    out_csv = os.path.join(HERE, "v4_gdc_diff_results.csv")
    ...
    print(f"\n=== GDC-TS on diffs, M4 daily ({len(ids)} series) ===")
