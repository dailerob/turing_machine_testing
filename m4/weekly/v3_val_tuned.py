"""V3: per-series val-tuned ensemble for M4 weekly.

Small candidate set (8 configs) to avoid h=13 validation overfitting.
"""
...
CONFIGS = [
    ...
    ('drift', 'drift', dict()),
    ('nn_L13_s0.25', 'nn',  dict(window_len=13, sigma_frac=0.25)),
    ('nn_L26_s0.25', 'nn',  dict(window_len=26, sigma_frac=0.25)),
    ('gdc_L26_s0.25_a0.99', 'gdc',
        dict(window_len=26, sigma_frac=0.25, alpha=0.99, theta=0.0)),
    ('gdc_L13_s0.25_a0.99', 'gdc',
        dict(window_len=13, sigma_frac=0.25, alpha=0.99, theta=0.0)),
    ('gdc_L52_s0.25_a0.99', 'gdc',
        dict(window_len=52, sigma_frac=0.25, alpha=0.99, theta=0.0)),
    ('gdc_L26_s0.10_a0.99', 'gdc',
        dict(window_len=26, sigma_frac=0.10, alpha=0.99, theta=0.0)),
]
...
def run_series(args):
    ...
    rows = [dict(sid=sid, model='val_tuned', best_cfg=best_name,
                 ...
                 test_smape=smape(test, pred),
                 naive_smape=smape(test, naive_last(train, h)))]
    # Also report each fixed config for comparison
    for name, kind, cfg in CONFIGS:
...
def main():
    ...
    print(f"\n=== Per-model test sMAPE on M4 weekly ({len(ids)} series) ===")
