"""NPI-style addition program + trace generator — BASE 2 (binary).

Same 4-subroutine decomposition as `npi_program.py` (ADD/ADD1/CARRY/LSHIFT),
but in base 2. The addition table shrinks from 10×10×2=200 cells to 2×2×2=8
cells, so the random-sampling coverage problem should be much easier.

Trace row format is identical: 6 ints [obs_p1, obs_p2, obs_p3, obs_p4,
action_type, arg]. obs columns take values {0, 1, BLANK}. The WRITE_p4
arg encoding still uses arg = 1 + digit (so arg ∈ {1, 2} in binary).
INIT_A/INIT_B args are digit values (0 or 1).
"""
...
# Re-use the same value+constant encoding as base 10 so we can share
# embeddings / argmax tables across both bases. BLANK still = 10.
BLANK = 10
...
def WRITE_p4(v: int) -> int:
    assert 0 <= v < BASE
    return 1 + v   # WRITE_p4_0=1, WRITE_p4_1=2
...
class _NPIRunnerB2:
    def __init__(self, a: int, b: int):
        ...
        a_w = len(bin(a)[2:]); b_w = len(bin(b)[2:])
        max_in = max(a_w, b_w)
...
def decode_output_from_trace(trace: np.ndarray) -> int:
    ...
    for row in trace:
        ...
        if at == AT_MOVE:
            ...
            if arg == 1: cells[p4_col] = 0
            elif arg == 2: cells[p4_col] = 1
    if not cells:
        return 0
...
if __name__ == "__main__":
    cases = [(0, 0), (1, 0), (0, 1), (1, 1), (3, 5), (7, 1),
             (15, 1), (5, 3), (10, 6), (255, 1), (1, 255), (170, 85)]
    all_ok = True
    for a, b in cases:
