...
Same layout / colour as v1, but the edge labels use a more typeset look:
the blank symbol is the standard ⊔, the read→write step uses a unicode
arrow, and the head direction uses ▶ / ◀ instead of the letters R / L.
"""
...
def main():
    ...
    for (src, dst), labels in cross.items():
        draw_edge(ax, POSITIONS[src], POSITIONS[dst], labels,
                  ...
                  label_format=label_arrow, label_size=7.5)
    ...
    # Tiny legend for the label notation
    ax.text(
        7.4, -1.2,
        r'edge:  $r \to w$   $\triangleright = $R   $\triangleleft = $L   $\sqcup = $blank',
        ha='right', va='center',
    ...
    ax.set_aspect('equal')
    ax.set_axis_off()
    fig.suptitle('Binary-alphabet adder TM — typeset edge labels',
